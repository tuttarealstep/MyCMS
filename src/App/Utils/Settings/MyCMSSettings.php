...
class MyCMSSettings
{
    ...
    function getSettingsName($id)
    {
        ...
        if (filter_var($filter_id, FILTER_VALIDATE_INT)) {
            ...
            $this->logger->addInfo('Error in getSettingsName() invalid $filter_id - MyCMSSettings');
            throw new MyCMSException('Error in getSettingsName() invalid $filter_id');
        }
    ...
    function saveSettings($settings_name = "", $settings_value = "", $createSetting = false)
    {
        $settings_name = filter_var($settings_name, FILTER_SANITIZE_STRING);
        $settings_value = filter_var($settings_value, FILTER_SANITIZE_STRING);
        $check = $this->database->single("SELECT COUNT(*) FROM my_cms_settings WHERE settings_name = :filter_settings_name", ["filter_settings_name" => $settings_name]);

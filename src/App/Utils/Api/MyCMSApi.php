    ...
    class MyCMSApi
    {
        ...
        function isApi($url = null, $return_array = false)
        {
            if (empty($url)) {
                ...
                $url = $this->container['functions']->fix_text(htmlspecialchars(substr((string)$_SERVER["REQUEST_URI"], 1)));
            } else {
                $url = $this->container['functions']->fix_text(htmlspecialchars($url));
            }
        ...
        function typeApi($url)
        {
            ...
            if (preg_match("/_api_/users//i", $url)) {
                ...
                if (is_numeric($match["1"])) {
                    $count = $this->container['database']->single("SELECT COUNT(settings_name) FROM my_cms_settings WHERE settings_id = :settings_id", array("settings_id" => $this->container['security']->my_sql_secure($match['1'])));
                } else {
                    $count = $this->container['database']->single("SELECT COUNT(settings_name) FROM my_cms_settings WHERE settings_name = :settings_name", array("settings_name" => $this->container['security']->my_sql_secure($match['1'])));
                }

                if ($count > 0) {
                    if (is_numeric($match["1"])) {
                        $settings = $this->container['database']->query("SELECT * FROM my_cms_settings WHERE settings_id = :settings_id", array("settings_id" => $this->container['security']->my_sql_secure($match['1'])));
                    } else {
                        $settings = $this->container['database']->query("SELECT * FROM my_cms_settings WHERE settings_name = :settings_name", array("settings_name" => $this->container['security']->my_sql_secure($match['1'])));
                    }

<?php
    ...
    class MyCMSTheme
    {
        ...
        public $script = [];
        public $meta_tag = [];
        public $index_error_style_array = [];
        ...
        {
            $path_file = $this->get_theme_path() . "/inc/functions.php";
            ...
            if (isset($_SESSION["staff"]["id"])) {
                ...
                if (isset($_SESSION['customizerThemeSession']['theme'])) {
                    if (isset($_GET["theme"])) {
                        if ($this->theme_exist($this->container["security"]->my_sql_secure($_GET["theme"]))) {
                            $this->loadThemeFunctions($this->container["security"]->my_sql_secure($_GET["theme"]));
                        } else {
                            $this->loadThemeFunctions("default");
                        }
                    }
                }
            }
         ...
         * @return bool
         */
        function loadThemeFunctions($theme = "default")
        {
         ...
         */
        //todo add load_theme in plugin system? for support custom engine or other??
        ...
        function load_theme($file, $param)
        {
            ...
            if ($this->twig_enabled == false && $this->get_style_info("require_twig_engine") == true) {
                //TODO send notify (theme need twig enabled)
            }

            if ($this->twig_enabled == true && $this->get_style_info("require_twig_engine") == false) {
            ...
            if ($this->twig_enabled) {
                ...
                if ($file == $style_info["style_error_page"]) {
                ...
                if ($file_found) {
                    ...
                    if (!empty($param)) {
                        foreach ($param as $key => $value) {
                            $_GET[ $key ] = $this->container['security']->my_sql_secure($value);
                        }
                    }
                    ...
                    if (isset($_SESSION['staff']['id'])) {
                        if (isset($_GET['custom_tags'])) {
                            foreach ($this->tag as $tag => $value) {
                                if (isset($_GET[ $tag ])) {
                                    $this->tag[ $tag ] = $this->container['security']->my_sql_secure($_GET[ $tag ]);
                                }
                            }
                        }
                    }
                    ...
                    if ($file_found_ext == ".php") {
                        ...
                        $page_loaded = $this->set_TAG_FUNCTIONS($page_loaded);
                    ...
                    header('Location: ' . HOST . '/' . $style_info["style_error_page"]);
        ...
        //todo add set_page in plugin system

        public function get_style_info($info)
        {
            $template = null;

            $path_file = $this->get_theme_path() . '/inc/info.php';
        ...
        {
            if (!empty($style)) {
                $test = $this->container['database']->iftrue("SELECT style_id FROM my_style WHERE style_path_name = :style_path_name", array("style_path_name" => $this->container['security']->my_sql_secure($style)));
                if ($test) {
                    $style_info = $this->container['database']->row("SELECT * FROM my_style WHERE style_path_name = :style_path_name", array("style_path_name" => $this->container['security']->my_sql_secure($style)));
        ...
        function set_page($page)
        {
            ...
            $page = $this->set_TAG_FUNCTIONS($page);
        ...
        public function get_meta_tag($page_name)
        {
            ...
            if ($this->container['settings']->get_settings_value('site_private') == 'true') {
                $this->no_robots();
            }
        }
        ...
        public function get_file($page, $name = null, $page_loader = false, $returnExtension = false)
        {
            $theme_path = $this->get_theme_path();
            if (!file_exists($theme_path)) {
                throw new MyCMSException("Theme not found!");
        ...
        public function is_maintenance($admin_check = true)
        {
            $maintenance = $this->container['settings']->get_settings_value('site_maintenance');

            if ($admin_check == true && $this->container['users']->staff_logged_in()) {
                $maintenance = false;
            } else {
                if ($maintenance == 'true') {
                    $maintenance = true;
        ...
        function remove_dir($dir)
        {
            if (is_dir($dir)) {
                ...
                foreach ($files as $file) {
                    if ($file != "." && $file != "..") {
                        $this->container['functions']->remove_dir("$dir/$file");
                    }
                }
        ...
        function installTheme($fileInfo)
        {
            ...
            $rndName = $this->container['security']->my_generate_random(5);
        ...
        {
            if (file_exists($dst)) {
                $this->container['functions']->remove_dir($dst);
            }
                ...
                switch ($command) {
                    ...
                    case 'my-admin-mode':
                        ...
                        while ($try == true) {
                            ...
                            $mail = htmlentities($this->container['security']->my_sql_secure($email));
                            $password = htmlentities($this->container['security']->my_sql_secure($password));
                            $login = $this->container['users']->login_admin($mail, $password, false);
                            if ($login["login"] == 1) {
                                $success = true;
                                $try = false;
                            } else {
        ...
        public function getThemeSettings($theme = "")
        {
            ...
            $settings = $this->container["settings"]->get_settings_value("theme_settings_$theme");
            if ($settings === false) {
                return [];
            }

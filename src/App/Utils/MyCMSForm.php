<?php
    if (isset($_POST['admin-login'])) {
        $mail = htmlentities($App->container['security']->my_sql_secure($_POST['email']));
        $password = htmlentities($App->container['security']->my_sql_secure($_POST['password']));

        if (isset($_POST['remember'])) {
            $remember = htmlentities($App->container['security']->my_sql_secure($_POST['remember']));
        } else {
            $remember = null;
        }

        if ($remember == "remember_t") {
            $login = $App->container['users']->login_admin($mail, $password, true);
            if ($login["login"] == 1) {
            ...
            $login = $App->container['users']->login_admin($mail, $password, false);
            if ($login["login"] == 1) {
                header("location: " . HOST . "/my-admin/index");
                exit;
            } else {
    ...
    if (isset($_POST['login'])) {
        $mail = htmlentities($App->container['security']->my_sql_secure($_POST['mail']));
        $password = htmlentities($App->container['security']->my_sql_secure($_POST['password']));
        if (isset($_POST['remember'])) {
            $remember = htmlentities($App->container['security']->my_sql_secure($_POST['remember']));
        } else {
    ...
    if (isset($_POST['register'])) {
        ...
        $register = $App->container['users']->register($App->container['security']->my_sql_secure($email), $App->container['security']->my_sql_secure($password), $App->container['security']->my_sql_secure($name), $App->container['security']->my_sql_secure($surname));
    ...
    if (isset($_POST['postCOMMENT'])) {
        if ($App->container['users']->user_logged_in()) {
    ...
    if (isset($_POST['search'])) {
        ...
        $searchstring = $App->container['security']->my_sql_secure(htmlentities($_POST['searchform']));
    ...
    if (isset($_POST['save_settings_general'])) {
        ...
        if ($App->container['users']->staff_logged_in()) {
        ...
        if ($App->container['users']->staff_logged_in()) {
            ...
            if ($user_rank >= 3) {
                ...
                if ($settings_blog_private == 'true') {
                    $App->container['blog']->set_private(true);
                } else {
                    $App->container['blog']->set_private(false);
                }
        ...
        if ($App->container['users']->staff_logged_in()) {
            ...
            if ($user_rank >= 3) {
                ...
                if (isset($mycms_xml->command['value'])) {
                    if ($App->container['security']->my_cms_xml_command($mycms_xml->command['value'])) {
                        if ($mycms_xml->command['value'] == "add_new_language") {
                            if (empty($mycms_xml->command->language_name)) {
                                ...
                                if (empty($mycms_xml->command->language_language)) {
                                    ...
                                    $App->container['database']->query("INSERT INTO my_language (language_name,language_language) VALUES (:language_name, :language_language)", array('language_name' => $mycms_xml->command->language_name, 'language_language' => $mycms_xml->command->language_language));
                                }
                            }
                        } elseif ($mycms_xml->command['value'] == "remove_language") {
                            if (empty($mycms_xml->command->language_language)) {
                            } else {
                                $App->container['database']->query('DELETE FROM my_language WHERE language_language = :language_language LIMIT 1', array('language_language' => $mycms_xml->command->language_language));
                            }
                        } elseif ($mycms_xml->command['value'] == "add_new_style") {
                            if (empty($mycms_xml->command->style_name)) {
                            } else {
                                $App->container['database']->query("INSERT INTO my_style (style_name) VALUES (:style_name)", array('style_name' => $mycms_xml->command->style_name));
                            }
                        } elseif ($mycms_xml->command['value'] == "remove_style") {
                            if (empty($mycms_xml->command->style_name)) {
                            } else {
                                $App->container['database']->query('DELETE FROM my_style WHERE style_path_name = :style_path_name LIMIT 1', array('style_path_name' => $mycms_xml->command->style_path_name));
                            }
                        }
                    }
                }
    ...
    if (isset($_POST['import_page_json'])) {
        ...
        if ($App->container['users']->staff_logged_in()) {
            ...
            if ($user_rank >= 3) {
                ...
                if ($mycms_json != false) {
                    $pages_title = $App->container['functions']->add_space(addslashes($mycms_json['pageTITLE']));
                    ...
                    if (substr($pages_menu_id, 5, strlen($pages_menu_id) - 5) == $pages_title) {
                        $find_url = $App->container['database']->single("SELECT COUNT(*) FROM my_page WHERE pageURL = :simulate_url", array("simulate_url" => $page_url));
                        if ($find_url > 0) {
                            $page_url = "{@siteURL@}/" . $App->container['security']->my_generate_random(5) . $pages_title;
                        }
    ...
    if (isset($_POST['page_settings_user_save_button'])) {
        if ($App->container['users']->staff_logged_in()) {
            $password = htmlentities($App->container['security']->my_sql_secure($_POST['password']));
            $new_password = htmlentities($App->container['security']->my_sql_secure($_POST['new_password']));
            $password_repeat = htmlentities($App->container['security']->my_sql_secure($_POST['password_repeat']));


            $validate_password = $App->container['users']->validate("password", $new_password);

            $user_data = $App->container['users']->get_user_data($_SESSION['staff']['id']);
            ...
            $password = $App->container['security']->my_cms_security_create_password($new_password);

    ...
    class MyCMSAdmin
    {
        ...
        function checkNotification()
        {
            if ($this->container['users']->isAdmin()) {
                if (!isset($_SESSION['my-admin']['notification']['dashboard'])) {
                    if ($this->container['theme']->there_is_new_update()) {
                        $_SESSION['my-admin']['notification']['dashboard'] = $this->container['theme']->there_is_new_update(false);
                    } else {
        ...
        function getMenu($menuId, $return = false)
        {
            ...
            if (isset($this->menuArrayCallback[ $menuId ])) {
                ...
                $this->container['plugins']->addEvent("my_plugin_" . strtolower($this->container["functions"]->add_space($menuId)), $this->menuArrayCallback[ $menuId ]);
                $this->menuArray[ $menuId ]["link"] = "/my-admin/my_plugin/" . strtolower($this->container["functions"]->add_space($menuId));
            }
        ...
        function checkEvents($menuId)
        {
            if (isset($this->subMenuArrayCallback[ $menuId ])) {
                $this->container['plugins']->addEvent("my_plugin_" . strtolower($this->container["functions"]->add_space($menuId)), $this->subMenuArrayCallback[ $menuId ]["callBack"]);
                $this->subMenuArray[ $menuId ]["link"] = "/my-admin/my_plugin/" . strtolower($this->container["functions"]->add_space($menuId));
        ...
        function addAdminPage($pageNAME, $callBack)
        {
            if (is_callable($callBack)) {
                //die(strtolower($this->container["functions"]->add_space($pageNAME)));
                $this->container['plugins']->addEvent("my_plugin_" . strtolower($this->container["functions"]->add_space($pageNAME)), $callBack);
            }
        }

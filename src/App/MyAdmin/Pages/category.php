...
define('PAGE_NAME', $this->container['languages']->ea('page_category_name', '1'));
    ...
    if ($_POST['ifchecked'] == 'delete') {
        if (!empty($_POST['check_list'])) {
            foreach ($_POST['check_list'] as $select) {
                $this->container['database']->query('DELETE FROM my_blog_category WHERE catID = :select', ['select' => $select]);
                $info = '<div class="row"><div class="alert alert-success">' . $this->container['languages']->ea('page_category_delete_successfull', '1') . '</div>';
            }
        } else {
            $info = '<div class="row"><div class="alert alert-danger">' . $this->container['languages']->ea('page_category_delete_empty_checklist', '1') . '</div>';
        }
    }
}
if (isset($_POST['newcategory'])) {
    if (!empty($_POST['name'])) {
        ...
        if ($finder == true) {
            $info = '<div class="alert alert-danger">' . $this->container['languages']->ea('page_category_error_category_in_use', '1') . '</div>';
            $name = '';
            $description = $this->container['security']->mySqlSecure($_POST['description']);
        } else {
            $name = addslashes($_POST['name']);
            $description = addslashes($_POST['description']);
            $this->container['database']->query("INSERT INTO my_blog_category (catNAME,catDESCRIPTION) VALUES (:name, :description)", ['name' => $name, 'description' => $description]);
            $info = '<div class="alert alert-success">' . $this->container['languages']->ea('page_category_addedd_succesful', '1') . '</div>';
            $name = '';
            $description = '';
        }
    } else {
        $info = '<div class="alert alert-danger">' . $this->container['languages']->ea('page_category_delete_empty_name', '1') . '</div>';
...
<div class="container">
    <div class="row">
        <div class="col-lg-12">
            ...
            <h1 class="h1PagesTitle"><?php $this->container['languages']->ea('page_category_header'); ?></h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
    <div class="row">
        <div class="col-lg-8">
            <div class="table-responsive">
                <form action="" method="post">
                    <table class="table table-striped table-bordered table-hover" id="tables_posts">
                        <thead>
                        <tr>
                            <th><?php $this->container['languages']->ea('page_category_table_name'); ?></th>
                            <th><?php $this->container['languages']->ea('page_category_table_description'); ?></th>
                            <th><?php $this->container['languages']->ea('page_category_table_post'); ?></th>
                            <th><?php $this->container['languages']->ea('page_category_table_select'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php
                        ...
                        foreach ($category as $categoryinfo) {
                            ...
                            <tr>
                                ...
                                <td><?php echo $this->container['database']->single("SELECT COUNT(*) FROM my_blog WHERE postCATEGORY = '" . $categoryinfo['catNAME'] . "'"); ?></td>
            ...
            <div class="col-lg-6">
                ...
                <select name="ifchecked" class="form-control">
                    <option value="delete"><?php $this->container['languages']->ea('page_category_check_delete'); ?></option>
                </select>
            </div>
            <div class="col-lg-3">
                <p>&nbsp;</p>
                <button type="submit" name="execute"
                        class="btn btn-danger"><?php $this->container['languages']->ea('page_category_check_button'); ?></button>
            </div>
            </form>
        </div>
        <!-- /.col-lg-12 -->
        ...
        <div class="col-lg-4">
            <div class="b_panel panel">
                <div class="panel-heading">
                    <h1 class="panel-title text-center"><?php $this->container['languages']->ea('page_category_add_new_category'); ?></h1>
                </div>
                <form action="" method="post">
                    <div class="panel-body b_panel-body">
                        <div class="panel-body-padding">
                            <input type="text" name="name" class="form-control b_form-control" maxlength="100"
                                   placeholder="<?php $this->container['languages']->ea('page_category_add_new_category_name'); ?>"
        ...
        transition: border-color ease-in-out .15s, box-shadow ease-in-out .15s;
                                      ...
                                      placeholder="<?php $this->container['languages']->ea('page_category_add_new_category_description'); ?>"><?php echo isset($description) ? $description : "" ?></textarea>
                            <br/>
                        </div>

                    </div>
                    <button type="submit" name="newcategory" class="btn btn-primary btn-block b_btn "
                            style="/*border-bottom-left-radius: 4px; border-bottom-right-radius: 4px;*/"><?php $this->container['languages']->ea('page_category_add_new_category_button'); ?></button>
                </form>
            </div>
        </div>
    </div>
